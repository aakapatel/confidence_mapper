...
#include "confidence_mapping/ThreadSafeDataWrapper.hpp"
...
/**
 * @brief An Input feeds data to ConfidenceMapping callbacks. E.g it holds a subscription to an sensor source and registered an appropriate
 * ConfidenceMapping callback.
 */
class Input {
 public:
  template <typename MsgT>
  using CallbackT = void (ConfidenceMapping::*)(MsgT, bool, const SensorProcessorBase::Ptr&);
   ...
   */
  template <typename MsgT>
  void registerCallback(ConfidenceMapping& map, CallbackT<MsgT> callback);
...
template <typename MsgT>
void Input::registerCallback(ConfidenceMapping& map, CallbackT<MsgT> callback) {
  const Parameters parameters{parameters_.getData()};
