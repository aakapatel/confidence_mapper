...
/*!
 * Confidence map stored as grid map handling elevation, height, variance,
 * color, confidence etc.
 */
class ConfidenceMap {
public:
  /*!
   ...
   * @param length the side lengths in x, and y-direction of the confidence map
   * [m].
   * @param resolution the cell size in [m/cell].
   * @param position the 2d position of the confidence map in the confidence map
   * frame [m].
  ...
  /*!
   * Publishes the (latest) raw confidence map. Optionally, if a postprocessing
  ...
  /*!
   * Publishes the fused confidence map. Takes the latest available fused
   * confidence map, does not trigger the fusion process.
   * @return true if successful.
   */
  ...
  /*!
   * Get the pose of the confidence map frame w.r.t. the inertial parent frame
   * of the robot (e.g. world, map etc.).
   * @return pose of the confidence map frame w.r.t. the parent frame of the
   * robot.
   */
...
private:
  ...
  //! Postprocessor pipeline for filtering and filling pipeline.
  ...
  //! Pose of the confidence map frame w.r.t. the inertial parent frame of the
  //! robot (e.g. world, map etc.).
  kindr::HomTransformQuatD pose_;

  //! ROS publishers. Publishing of the raw confidence map is handled by the
  //! postprocessing pool.
  rclcpp::Publisher<grid_map_msgs::msg::GridMap>::SharedPtr
      confidenceMapFusedPublisher_;
  rclcpp::Publisher<grid_map_msgs::msg::GridMap>::SharedPtr
