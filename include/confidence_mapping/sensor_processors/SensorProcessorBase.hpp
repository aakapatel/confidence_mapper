...
class SensorProcessorBase {
 ...
 protected:
  ...
  /*!
   * Computes the confidence map height variances for each point in a point cloud with the
   ...
   * @param[out] variances the confidence map height variances.
   * @return true if successful.
   */
  ...
  //! Rotation from (confidence) Map to Base frame (C_BM)
