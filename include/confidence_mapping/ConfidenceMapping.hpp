...
#include "confidence_mapping/RobotMotionMapUpdater.hpp"
#include "confidence_mapping/WeightedEmpiricalCumulativeDistributionFunction.hpp"
#include "confidence_mapping/input_sources/InputSourceManager.hpp"
#include "confidence_mapping/sensor_processors/SensorProcessorBase.hpp"
#include <exception>
...
/*!
 * The confidence mapping main class. Coordinates the ROS interfaces, the timing,
...
class ConfidenceMapping {
 public:
   ...
   */
  explicit ConfidenceMapping(std::shared_ptr<rclcpp::Node>& nodeHandle);

  /*!
   * Destructor.
   */
  virtual ~ConfidenceMapping();

  /*!
   * Callback function for new data to be added to the confidence map.
   *
  ...
  /*!
   * ROS service callback function to return a submap of the fused confidence map.
   *
  ...
  /*!
   * ROS service callback function to return a submap of the raw confidence map.
   *
  ...
  /*!
   * ROS service callback function to enable updates of the confidence map.
   *
   * @param request     The ROS service request.
   * @param response    The ROS service response.
   * @return true if successful.
  ...
  /*!
   * ROS service callback function to disable updates of the confidence map.
   *
   * @param request     The ROS service request.
   * @param response    The ROS service response.
   * @return true if successful.
  ...
  /*!
   * ROS service callback function to clear all data of the confidence map.
   *
   * @param request     The ROS service request.
   * @param response    The ROS service response.
   * @return true if successful.
  ...
  /*!
   * ROS service callback function to allow for setting the individual layers of the confidence map through a service call.
 ...
 private:
  ...
  /*!
   * Update the confidence map from the robot motion up to a certain time.
   *
  ...
  /*!
   * Initializes a submap around the robot of the confidence map with a constant height.
   */
  bool initializeConfidenceMap();
 ...
 protected:
  ...
  //! Frame ID of the confidence map
  ...
  //! Point which the confidence map follows.
  ...
  //! Enables initialization of the confidence map
  bool initializeConfidenceMap_;

  //! Enum to choose the initialization method
  int initializationMethod_;

  //! Width of submap of the confidence map with a constant height
  double lengthInXInitSubmap_;

  //! Height of submap of the confidence map with a constant height
  double lengthInYInitSubmap_;

  //! Margin of submap of the confidence map with a constant height
  double marginInitSubmap_;

  //! Target frame to get the init height of the confidence map
