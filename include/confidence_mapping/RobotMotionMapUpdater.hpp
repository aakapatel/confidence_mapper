...
class RobotMotionMapUpdater {
 public:
  ...
  /*!
   * Computes the model update for the confidence map based on the pose covariance and
   * adds the update to the map.
   * @param[in] map the confidence map to be updated.
   ...
   * @return true if successful.
   */
  bool update(ConfidenceMap& map, const Pose& robotPose, const PoseCovariance& robotPoseCovariance, const rclcpp::Time& time);
