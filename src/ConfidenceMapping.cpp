...
#include "confidence_mapping/sensor_processors/LaserSensorProcessor.hpp"
#include "confidence_mapping/sensor_processors/PerfectSensorProcessor.hpp"
#include "confidence_mapping/sensor_processors/StereoSensorProcessor.hpp"
#include "confidence_mapping/sensor_processors/StructuredLightSensorProcessor.hpp"

namespace confidence_mapping {

ConfidenceMapping::ConfidenceMapping(std::shared_ptr<rclcpp::Node> &nodeHandle)
    : nodeHandle_(nodeHandle), inputSources_(nodeHandle_),
      ...
      initializeConfidenceMap_(true), initializationMethod_(0),
  ...
  RCLCPP_INFO(nodeHandle_->get_logger(), "Confidence mapping node started.");
...
void ConfidenceMapping::setupSubscribers() { // Handle deprecated
  ...
  if (configuredInputSources) {
    inputSources_.registerCallbacks(
        *this,
        std::make_pair("pointcloud", &ConfidenceMapping::pointCloudCallback));
...
void ConfidenceMapping::setupServices() {
  // Multi-threading for fusion.
  fusionTriggerService_ = nodeHandle_->create_service<std_srvs::srv::Empty>(
      "trigger_fusion",
      std::bind(&ConfidenceMapping::fuseEntireMapServiceCallback, this,
  ...
  fusedSubmapService_ =
      nodeHandle_->create_service<grid_map_msgs::srv::GetGridMap>(
          "get_submap",
          std::bind(&ConfidenceMapping::getFusedSubmapServiceCallback, this,
                    std::placeholders::_1, std::placeholders::_2),
          rclcpp::ServicesQoS().get_rmw_qos_profile(), fusionServiceGroup_);
  ...
  rawSubmapService_ =
      nodeHandle_->create_service<grid_map_msgs::srv::GetGridMap>(
          "get_raw_submap",
          std::bind(&ConfidenceMapping::getRawSubmapServiceCallback, this,
                    std::placeholders::_1, std::placeholders::_2),
          rclcpp::ServicesQoS().get_rmw_qos_profile(), fusionServiceGroup_);

  clearMapService_ = nodeHandle_->create_service<std_srvs::srv::Empty>(
      "clear_map", std::bind(&ConfidenceMapping::clearMapServiceCallback, this,
                             std::placeholders::_1, std::placeholders::_2,
                             std::placeholders::_3));
  enableUpdatesService_ = nodeHandle_->create_service<std_srvs::srv::Empty>(
      "enable_updates",
      std::bind(&ConfidenceMapping::enableUpdatesServiceCallback, this,
                std::placeholders::_1, std::placeholders::_2,
                std::placeholders::_3));
  disableUpdatesService_ = nodeHandle_->create_service<std_srvs::srv::Empty>(
      "disable_updates",
      std::bind(&ConfidenceMapping::disableUpdatesServiceCallback, this,
                std::placeholders::_1, std::placeholders::_2,
                std::placeholders::_3));
  maskedReplaceService_ =
      nodeHandle_->create_service<grid_map_msgs::srv::SetGridMap>(
          "masked_replace",
          std::bind(&ConfidenceMapping::maskedReplaceServiceCallback, this,
                    std::placeholders::_1, std::placeholders::_2));
  saveMapService_ =
      nodeHandle_->create_service<grid_map_msgs::srv::ProcessFile>(
          "save_map", std::bind(&ConfidenceMapping::saveMapServiceCallback, this,
                                std::placeholders::_1, std::placeholders::_2));
  loadMapService_ =
      nodeHandle_->create_service<grid_map_msgs::srv::ProcessFile>(
          "load_map", std::bind(&ConfidenceMapping::loadMapServiceCallback, this,
                                std::placeholders::_1, std::placeholders::_2));
}

void ConfidenceMapping::setupTimers() {
  ...
  // &ConfidenceMapping::mapUpdateTimerCallback);
  ...
  if (fusedMapPublishTimerDuration_.seconds() != 0.0) {
    fusedMapPublishTimer_ = rclcpp::create_timer(
        ...
        std::bind(&ConfidenceMapping::publishFusedMapCallback, this));
  }
  ...
  if (map_.enableVisibilityCleanup_ &&
    ...
    visibilityCleanupTimer_ = rclcpp::create_timer(
        ...
        std::bind(&ConfidenceMapping::visibilityCleanupCallback, this));
...
bool ConfidenceMapping::readParameters() {
  // ConfidenceMapping parameters.
  ...
  if (fusedMapPublishingRate == 0.0) {
    ...
    RCLCPP_WARN(
        ...
        "Rate for publishing the fused map is zero. The fused Confidence map "
  ...
  // ConfidenceMap parameters. TODO Move this to the Confidence map class.
  ...
  // Settings for initializing Confidence map
  nodeHandle_->declare_parameter("initialize_confidence_map", false);
  ...
  nodeHandle_->get_parameter("initialize_confidence_map",
                             initializeConfidenceMap_);
...
/*void ConfidenceMapping::runFusionServiceThread() {
...
/*void ConfidenceMapping::visibilityCleanupThread() {
  rclcpp::Rate loopRate(20);
...
void ConfidenceMapping::pointCloudCallback(
  ...
  if (!updatesEnabled_) {
    ...
    RCLCPP_WARN_THROTTLE(nodeHandle_->get_logger(), *(clock), 10,
                         "Updating of confidence map is disabled. (Warning "
                         "message is throttled, 10s.)");
    if (publishPointCloud) {
      map_.setTimestamp(nodeHandle_->get_clock()->now());
      map_.postprocessAndPublishRawConfidenceMap();
    }
    return;
  }
  ...
  if (!receivedFirstMatchingPointcloudAndPose_) {
    ...
    if (currentPointCloudTime < oldestPoseTime) {
      ...
      RCLCPP_INFO(nodeHandle_->get_logger(),
                  "First corresponding point cloud and pose found, confidence "
                  "mapping started. ");
  ...
  // RCLCPP_INFO(nodeHandle_->get_logger(), "ConfidenceMap received a point cloud
  // (%i points) for confidence mapping.", static_cast<int>(pointCloud->size()));
  RCLCPP_DEBUG(
      nodeHandle_->get_logger(),
      "ConfidenceMap received a point cloud (%i points) for confidence mapping.",
  ...
  if (map_.enableContinuousCleanup_) {
    RCLCPP_DEBUG(nodeHandle_->get_logger(),
                 "Clearing confidence map before adding new point cloud.");
    map_.clear();
  }

  // Add point cloud to confidence map.
  if (!map_.add(
    ...
    RCLCPP_ERROR(nodeHandle_->get_logger(),
                 "Adding point cloud to confidence map failed.");
    // resetMapUpdateTimer();
    return;
  }
  ...
  if (publishPointCloud) {
    ...
    if (isFusingEnabled()) {
      map_.fuseAll();
      map_.publishFusedConfidenceMap();
    }
  }
...
void ConfidenceMapping::mapUpdateTimerCallback() {
  if (!updatesEnabled_) {
    rclcpp::Clock clock;
    RCLCPP_WARN_THROTTLE(nodeHandle_->get_logger(), clock, 10,
                         "Updating of confidence map is disabled. (Warning "
  ...
  RCLCPP_WARN_THROTTLE(nodeHandle_->get_logger(), clock, 5,
                       "Confidence map is updated without data from the sensor. "
...
void ConfidenceMapping::publishFusedMapCallback() {
  if (!map_.hasFusedMapSubscribers()) {
    return;
  }
  RCLCPP_DEBUG(nodeHandle_->get_logger(),
               "Confidence map is fused and published from timer.");
  boost::recursive_mutex::scoped_lock scopedLock(map_.getFusedDataMutex());
  map_.fuseAll();
  map_.publishFusedConfidenceMap();
}

void ConfidenceMapping::visibilityCleanupCallback() {
  RCLCPP_DEBUG(nodeHandle_->get_logger(),
               "Confidence map is running visibility cleanup.");
  // Copy constructors for thread-safety.
  map_.visibilityCleanup(rclcpp::Time(lastPointCloudUpdateTime_, RCL_ROS_TIME));
}

bool ConfidenceMapping::fuseEntireMapServiceCallback(
...
bool ConfidenceMapping::isFusingEnabled() {
  return isContinuouslyFusing_ && map_.hasFusedMapSubscribers();
}

bool ConfidenceMapping::updatePrediction(const rclcpp::Time &time) {
  if (ignoreRobotMotionUpdates_) {
    return true;
  }
...
bool ConfidenceMapping::updateMapLocation() {
  RCLCPP_DEBUG(nodeHandle_->get_logger(),
               "Confidence map is checked for relocalization.");
...
bool ConfidenceMapping::getFusedSubmapServiceCallback(
  ...
  RCLCPP_DEBUG(
      nodeHandle_->get_logger(),
      "Confidence submap request: Position x=%f, y=%f, Length x=%f, y=%f.",
  ...
  RCLCPP_DEBUG(nodeHandle_->get_logger(),
               "Confidence submap responded with timestamp %f.",
               map_.getTimeOfLastFusion().seconds());
  return isSuccess;
}

bool ConfidenceMapping::getRawSubmapServiceCallback(
    std::shared_ptr<grid_map_msgs::srv::GetGridMap::Request> request,
    std::shared_ptr<grid_map_msgs::srv::GetGridMap::Response> response) {
  grid_map::Position requestedSubmapPosition(request->position_x,
                                             request->position_y);
  ...
  RCLCPP_DEBUG(
      nodeHandle_->get_logger(),
      "Confidence raw submap request: Position x=%f, y=%f, Length x=%f, y=%f.",
      requestedSubmapPosition.x(), requestedSubmapPosition.y(),
      requestedSubmapLength(0), requestedSubmapLength(1));
  boost::recursive_mutex::scoped_lock scopedLock(map_.getRawDataMutex());
...
bool ConfidenceMapping::disableUpdatesServiceCallback(
    const std::shared_ptr<rmw_request_id_t>,
    const std::shared_ptr<std_srvs::srv::Empty::Request>,
    std::shared_ptr<std_srvs::srv::Empty::Response>) {
...
bool ConfidenceMapping::enableUpdatesServiceCallback(
    const std::shared_ptr<rmw_request_id_t>,
    const std::shared_ptr<std_srvs::srv::Empty::Request>,
    std::shared_ptr<std_srvs::srv::Empty::Response>) {
...
bool ConfidenceMapping::initializeConfidenceMap() {
  if (initializeConfidenceMap_) {
    if (static_cast<confidence_mapping::InitializationMethods>(
            initializationMethod_) ==
        confidence_mapping::InitializationMethods::PlanarFloorInitializer) {
      ...
      try {
        ...
        RCLCPP_WARN(
            nodeHandle_->get_logger(),
            "Could not initialize confidence map with constant height. (This "
...
bool ConfidenceMapping::clearMapServiceCallback(
    const std::shared_ptr<rmw_request_id_t>,
    const std::shared_ptr<std_srvs::srv::Empty::Request>,
    std::shared_ptr<std_srvs::srv::Empty::Response>) {
  RCLCPP_INFO(nodeHandle_->get_logger(), "Clearing map...");
  bool success = map_.clear();
  success &= initializeConfidenceMap();
  RCLCPP_INFO(nodeHandle_->get_logger(), "Map cleared.");

  return success;
}

bool ConfidenceMapping::maskedReplaceServiceCallback(
  ...
  for (auto sourceLayerIterator = sourceMap.getLayers().begin();
    ...
    // Check if the layer exists in the confidence map
...
bool ConfidenceMapping::saveMapServiceCallback(
  ...
  map_.fuseAll();
  std::string topic =
      std::string(nodeHandle_->get_namespace()) + "/confidence_map";
...
bool ConfidenceMapping::loadMapServiceCallback(
    std::shared_ptr<grid_map_msgs::srv::ProcessFile::Request> request,
    std::shared_ptr<grid_map_msgs::srv::ProcessFile::Response> response) {
...
// void ConfidenceMapping::resetMapUpdateTimer() {
...
// void ConfidenceMapping::stopMapUpdateTimer() {
//   mapUpdateTimer_->cancel();
// }
