...
PostprocessingPipelineFunctor::PostprocessingPipelineFunctor(std::shared_ptr<rclcpp::Node> nodeHandle)
  ...
  if (!nodeHandle->has_parameter("filterChainParametersName_") ||
    ...
    RCLCPP_WARN(nodeHandle_->get_logger(), "Could not configure the filter chain. Will publish the raw confidence map without postprocessing!");
    return;
  }
...
grid_map::GridMap PostprocessingPipelineFunctor::operator()(GridMap& inputMap) {
  if (not filterChainConfigured_) {    
    RCLCPP_WARN_ONCE(nodeHandle_->get_logger(), "No postprocessing pipeline was configured. Forwarding the raw confidence map!");
    return inputMap;
  }
  RCLCPP_INFO(nodeHandle_->get_logger(), "performing Post processing");
  grid_map::GridMap outputMap;
  if (not filterChain_.update(inputMap, outputMap)) {
    RCLCPP_ERROR(nodeHandle_->get_logger(), "Could not perform the grid map filter chain! Forwarding the raw confidence map!");
    return inputMap;
  }
...
void PostprocessingPipelineFunctor::publish(const GridMap& gridMap) const {
  ...
  RCLCPP_DEBUG(nodeHandle_->get_logger(), "Confidence map raw has been published.");
}
