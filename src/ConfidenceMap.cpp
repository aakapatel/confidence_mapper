...
#include "confidence_mapping/ConfidenceMap.hpp"
#include "confidence_mapping/ConfidenceMapFunctors.hpp"
#include "confidence_mapping/PointXYZRGBConfidenceRatio.hpp"
#include "confidence_mapping/WeightedEmpiricalCumulativeDistributionFunction.hpp"
...
ConfidenceMap::ConfidenceMap(std::shared_ptr<rclcpp::Node> nodeHandle)
    : nodeHandle_(nodeHandle),
      rawMap_({"elevation", "variance", "horizontal_variance_x", "horizontal_variance_y", 
      "horizontal_variance_xy", "color", "confidence", "time","dynamic_time", "lowest_scan_point", 
      "sensor_x_at_lowest_scan", "sensor_y_at_lowest_scan", "sensor_z_at_lowest_scan"}),
      fusedMap_({"elevation", "upper_bound", "lower_bound", "color", "confidence"}),
...
ConfidenceMap::~ConfidenceMap() = default;

bool ConfidenceMap::add(const PointCloudType::Ptr pointCloud, Eigen::VectorXf& pointCloudVariances, const rclcpp::Time& timestamp,
  ...
  if (static_cast<unsigned int>(pointCloud->size()) != static_cast<unsigned int>(pointCloudVariances.size())) {
    RCLCPP_ERROR(nodeHandle_->get_logger(), "ConfidenceMap::add: Size of point cloud (%i) and variances (%i) do not agree.", (int)pointCloud->size(),
  ...
  // RCLCPP_INFO(nodeHandle_->get_logger(), "ConfidenceMap::add: Initializing map.");
  ...
  // RCLCPP_INFO(nodeHandle_->get_logger(), "ConfidenceMap::add: Storing references.");
  ...
  auto& confidenceLayer = rawMap_["confidence"];
  ...
  for (unsigned int i = 0; i < pointCloud->size(); ++i) {
    ...
    if (!rawMap_.getIndex(position, index)) {
      continue;  // Skip this point if it does not lie within the confidence map.
    }
    ...
    auto& confidence = confidenceLayer(index(0), index(1));
    ...
    if (!isValid) {
      // No prior information in confidence map, use measurement.
      // RCLCPP_INFO(nodeHandle_->get_logger(), "No prior information in confidence map, using measurement.");
      ...
      if (point.intensity > 0.3) {
        ...
        color = point.intensity;
        confidence = point.intensity;
      }
      
      // Horrible hack to get shit working with InPaintFilters // can only accept elevation for some unknown reasons. 
      
      // if (point.intensity > 0.3) {
      //   elevation = 0.9;
      // } else {
      //   elevation = point.intensity;
      // }
    ...
    // RCLCPP_INFO(nodeHandle_->get_logger(), "Fusing measurement with confidence map data.");
    // Fuse measurement with confidence map data. aka kalman filter
    elevation =
...
bool ConfidenceMap::update(const grid_map::Matrix& varianceUpdate, const grid_map::Matrix& horizontalVarianceUpdateX,
...
bool ConfidenceMap::fuse(const grid_map::Index& topLeftIndex, const grid_map::Index& size) {
  RCLCPP_DEBUG(nodeHandle_->get_logger(), "Fusing confidence map...");
  ...
  // Copy raw confidence map data for safe multi-threading.
  boost::recursive_mutex::scoped_lock scopedLockForRawData(rawMapMutex_);
  ...
  for (grid_map::SubmapIterator areaIterator(rawMapCopy, topLeftIndex, size); !areaIterator.isPastEnd(); ++areaIterator) {
    ...
    if (i == 0) {
      ...
      fusedMap_.at("confidence", *areaIterator) = rawMapCopy.at("confidence", *areaIterator);
      continue;
    }
  ...
  const std::chrono::duration<double> duration = std::chrono::system_clock::now() - methodStartTime;
  RCLCPP_DEBUG(nodeHandle_->get_logger(), "Confidence map has been fused in %f s.", duration.count());
...
void ConfidenceMap::visibilityCleanup(const rclcpp::Time& updatedTime) {
...
void ConfidenceMap::move(const Eigen::Vector2d& position) {
  boost::recursive_mutex::scoped_lock scopedLockForRawData(rawMapMutex_);
  std::vector<grid_map::BufferRegion> newRegions;

  if (rawMap_.move(position, newRegions)) {
    RCLCPP_DEBUG(nodeHandle_->get_logger(), "Confidence map has been moved to position (%f, %f).", rawMap_.getPosition().x(), rawMap_.getPosition().y());
...
void ConfidenceMap::setRawSubmapHeight(const grid_map::Position& initPosition, float mapHeight, double lengthInXSubmap,
...
bool ConfidenceMap::postprocessAndPublishRawConfidenceMap() {
...
bool ConfidenceMap::publishFusedConfidenceMap() {
  ...
  confidenceMapFusedPublisher_->publish(std::move(message));
  RCLCPP_DEBUG(nodeHandle_->get_logger(), "Confidence map (fused) has been published.");
  return true;
}

bool ConfidenceMap::publishVisibilityCleanupMap() {
  ...
  visibilityCleanupMapCopy.erase("confidence");
...
void ConfidenceMap::underlyingMapCallback(const grid_map_msgs::msg::GridMap::SharedPtr underlyingMap) {
  ...
  if (underlyingMap_.getFrameId() != rawMap_.getFrameId()) {
    RCLCPP_ERROR_STREAM(nodeHandle_->get_logger(), "The underlying map does not have the same map frame ('" << underlyingMap_.getFrameId() << "') as the confidence map ('"
                                                                              << rawMap_.getFrameId() << "').");
    return;
  }
  ...
  if (!underlyingMap_.exists("confidence")) {
    underlyingMap_.add("confidence", 0.0);
  }
...
void ConfidenceMap::setGeometry(const grid_map::Length& length, const double& resolution, const grid_map::Position& position) {
  boost::recursive_mutex::scoped_lock scopedLockForRawData(rawMapMutex_);
  boost::recursive_mutex::scoped_lock scopedLockForFusedData(fusedMapMutex_);
  rawMap_.setGeometry(length, resolution, position);
  fusedMap_.setGeometry(length, resolution, position);
  RCLCPP_INFO_STREAM(nodeHandle_->get_logger(), "Confidence map grid resized to " << rawMap_.getSize()(0) << " rows and " << rawMap_.getSize()(1) << " columns.");
}

float ConfidenceMap::cumulativeDistributionFunction(float x, float mean, float standardDeviation) {
  return 0.5 * erfc(-(x - mean) / (standardDeviation * sqrt(2.0)));
}

bool ConfidenceMap::hasFusedMapSubscribers() const {
  return confidenceMapFusedPublisher_->get_subscription_count() >= 1;
}

//sets frame id for both maps.
void ConfidenceMap::setFrameId(const std::string& frameId) {
  rawMap_.setFrameId(frameId);
  fusedMap_.setFrameId(frameId);
}

//gives time in nanoseconds (of type RCL_TIME) to the maps.
void ConfidenceMap::setTimestamp(rclcpp::Time timestamp) {
  rawMap_.setTimestamp(timestamp.nanoseconds());
  fusedMap_.setTimestamp(timestamp.nanoseconds());
}

//gets frame id from map raw.
const std::string& ConfidenceMap::getFrameId() {
  return rawMap_.getFrameId();
}

bool ConfidenceMap::hasRawMapSubscribers() const {
  return postprocessorPool_.pipelineHasSubscribers();
}

bool ConfidenceMap::clean() {
  boost::recursive_mutex::scoped_lock scopedLockForRawData(rawMapMutex_);
...
void ConfidenceMap::resetFusedData() {
  boost::recursive_mutex::scoped_lock scopedLockForFusedData(fusedMapMutex_);
  fusedMap_.clearAll();
  fusedMap_.resetTimestamp();
}

bool ConfidenceMap::fuseAll() {
  RCLCPP_DEBUG(nodeHandle_->get_logger(), "Requested to fuse entire confidence map.");
  boost::recursive_mutex::scoped_lock scopedLock(fusedMapMutex_);
  return fuse(grid_map::Index(0, 0), fusedMap_.getSize());
}

bool ConfidenceMap::fuseArea(const Eigen::Vector2d& position, const Eigen::Array2d& length) {
  RCLCPP_DEBUG(nodeHandle_->get_logger(), "Requested to fuse an area of the confidence map with center at (%f, %f) and side lengths (%f, %f)", position[0], position[1],
            length[0], length[1]);
...
grid_map::GridMap& ConfidenceMap::getRawGridMap() {
  return rawMap_;
}

void ConfidenceMap::setRawGridMap(const grid_map::GridMap& map) {
  boost::recursive_mutex::scoped_lock scopedLockForRawData(rawMapMutex_);
  rawMap_ = map;
}

grid_map::GridMap& ConfidenceMap::getFusedGridMap() {
  return fusedMap_;
}

void ConfidenceMap::setFusedGridMap(const grid_map::GridMap& map) {
  boost::recursive_mutex::scoped_lock scopedLockForFusedData(fusedMapMutex_);
  fusedMap_ = map;
}

rclcpp::Time ConfidenceMap::getTimeOfLastUpdate() {
  return rclcpp::Time(rawMap_.getTimestamp(), RCL_ROS_TIME);
}

rclcpp::Time ConfidenceMap::getTimeOfLastFusion() {
  boost::recursive_mutex::scoped_lock scopedLock(fusedMapMutex_);
  return rclcpp::Time(fusedMap_.getTimestamp(), RCL_ROS_TIME);
}

const kindr::HomTransformQuatD& ConfidenceMap::getPose() {
  return pose_;
}

bool ConfidenceMap::getPosition3dInRobotParentFrame(const Eigen::Array2i& index, kindr::Position3D& position) {
...
boost::recursive_mutex& ConfidenceMap::getFusedDataMutex() {
  return fusedMapMutex_;
}

boost::recursive_mutex& ConfidenceMap::getRawDataMutex() {
  return rawMapMutex_;
}
